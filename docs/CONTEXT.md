...
**Transmission Mode**:
A named channel of contact-based transmission (e.g. `respiratory`,
...
The `default_contacts_matrix` entry in `contact_matrices.yaml`: the fallback
contact matrix used for any (venue type OR encounter type, Transmission
Mode) pair with no dedicated entry — the same default applies whether the
lookup key is a physical `venue_type_id` or a virtual-encounter
`encounter_type_id`; there is no separate default for virtual encounters.
Must be present explicitly in the YAML file; a missing entry fails config
load rather than substituting an opaque hardcoded value. May be declared
flat (one matrix shared across all modes) or per-mode via a `modes:` block
matching the per-venue schema. The per-mode default is re-checked against
the disease's own mode list at `Simulator` construction
(`ContactMatrixConfig::finalizeDefaultModeMatrices`), independent of whatever
`contact_matrices.mode_names` contains — a disease mode missing from every
per-venue matrix and `contact_matrices.mode_names` is fine (falls back to the
default); a disease mode missing from the default itself (no per-mode entry
and no flat `default_matrix`) is a fatal, loud error at that point.
_Avoid_: default matrix (ambiguous with the single flat `default_matrix`
field that only exists for the flat-format case).
