...
namespace detail {
inline bool config_loader_log_rank0() {
#ifdef USE_MPI
  int initialized = 0;
...
// Forward declarations for CSV utilities (defined below)
inline std::pair<std::vector<std::string>,
                 std::vector<std::vector<std::string>>>
parseCSVFile(const std::string& filepath);
inline int findColumn(const std::vector<std::string>& headers,
                      const std::string& name);

inline Config ConfigLoader::loadAll(const std::string& simulation_file) {
  Config config;
...
inline SimulationConfig ConfigLoader::loadSimulation(
    const std::string& filename) {
...
inline ScheduleConfig ConfigLoader::loadSchedule(const std::string& filename) {
  YAML::Node root = YAML::LoadFile(filename);
...
inline ContactMatrixConfig ConfigLoader::loadContactMatrices(
    const std::string& filename) {
  YAML::Node root = YAML::LoadFile(filename);
...
inline PerformanceConfig ConfigLoader::loadPerformance(
    const std::string& filename) {
...
inline ParallelConfig ConfigLoader::loadParallel(const std::string& filename) {
...
inline ActivityPreferenceConfig ConfigLoader::loadActivityPreferences(
    const std::string& filename) {
...
inline VaccinationConfig ConfigLoader::loadVaccination(
    const std::string& filename) {
  VaccinationConfig config;
  if (!std::filesystem::exists(filename)) {
    if (detail::config_loader_log_rank0()) {
...
inline CoordinatedEncounterConfig ConfigLoader::loadCoordinatedEncounters(
    const std::string& filename) {
