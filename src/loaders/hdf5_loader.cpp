...
// loader.world_, reserving property slots before delegating to
// loadVenuePropertiesForSpan for the per-type property fill.
...
WorldState HDF5Loader::loadDomainChunked(
  ...
  for (size_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
    ...
    for (const auto& span : pop_spans) {
      loadPersonsInSpan(loader, span, pop_partition_map, geo_units_vec,
                        population_property_names, property_indices_cache);
    }
    ...
    for (const auto& span : venue_spans) {
      loadVenuesInSpan(loader, span, venue_partition_map, geo_units_vec,
                       venue_type_prop_names, venue_property_indices_cache);
    }
    ...
    if (loader.groupExists("/activity_mappings/activity_map")) {
      ...
      for (const auto& span : rel_spans) {
        loadActivityMappingsInSpan(loader, span, local_person_idx_map);
      }
    }
  }
  ...
  loadVenueSubsets(loader, owned_geo_units);
