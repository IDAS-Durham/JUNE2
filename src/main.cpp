...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    // and write manifest.yaml. Rank 0 only.
    if (rank == 0) {
    ...
    if (seed_override >= 0) {
      config.simulation.random_seed = seed_override;
      if (rank == 0)
    ...
    if (config.simulation.random_seed != 0) {
      june::GlobalRNG::seed(config.simulation.random_seed);
    } else {
      june::GlobalRNG::seed(std::random_device{}());
      if (rank == 0) {
        std::cerr << "Warning: No random seed set, results will not be "
                     "reproducible"
                  << std::endl;
      }
    }
